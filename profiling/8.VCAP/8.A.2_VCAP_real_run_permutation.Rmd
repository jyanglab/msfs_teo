---
title: "Run VCAP through huskeR: permutation"
output: NULL
date: 11-26-2019
...
permute_pheno <- function( nperm=100){
  for(i in 1:1){
    p <- read.table(pheno[i])
    ...
    set.seed(123457)
    for(j in 1:nperm){
      ps$V3 <- sample(p$V3, nrow(p), replace=FALSE)
      psout <- paste0(gsub(".*\\/|.txt", "", pheno[i]), "_perm", j, ".txt")
      write.table(ps, psout, row.names=FALSE, col.names=FALSE, quote=FALSE, sep="\t")
    }
  }
}
