...
mybarplot <- function(df=cg){
  ...
  subcg1 <- cg1[, c("pretty_name", "Her_K1", "Her_K2", "Her_K3", "Her_K4", "Her_K5")]
  subcg1$h2 <- subcg1$Her_K1 + subcg1$Her_K4 + subcg1$Her_K5
...
cg <- read.csv("cache/rare_common_h2_CG.csv")
cg <- subset(cg, Her_ALL >0)
pdf("graphs/barplot_CG_teo-lr.pdf", width = 8, height=8)
mybarplot(df=cg)
dev.off()  

chg <- read.csv("cache/rare_common_h2_CHG.csv")
