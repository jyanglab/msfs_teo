...
fwrite(meth[mef==0, 1:3], file="Completely_unmeth.bed", row.names = F, col.names = F, sep="\t", quote=F)

## mef ==1, completely methylated tiles
fwrite(meth[mef==1, 1:3], file="Completely_meth.bed", row.names = F, col.names = F, sep="\t", quote=F)

## mef > 0.9 or mef < 0.1, rare epialleles
fwrite(meth[mef>0.9 | mef<0.1, 1:3], file="Rare_epi.bed", row.names = F, col.names = F, sep="\t", quote=F)
