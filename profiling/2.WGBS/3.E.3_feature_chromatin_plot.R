...
pwd <- list.files(path=c("largedata/lcache"),  pattern="chr1_chromtin", full.names = TRUE)
length(pwd)
out <- data.frame()
...
write.table(out, "cache/chrp_chromatin.csv", sep=",", row.names=FALSE)


#############
d <- read.csv("cache/chrp_chromatin.csv")
d$x <- as.numeric(as.character(gsub("V", "", d$var)))
d$feature <- as.character(d$feature)

d$status <- gsub(".*chromtin_|_.*", "", d$fid)
