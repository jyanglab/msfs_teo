...
pwd <- list.files(path=c("largedata/lcache"),  pattern="chr1_TE", full.names = TRUE)
length(pwd)
out <- data.frame()
...
write.table(out, "cache/chrp_TE.csv", sep=",", row.names=FALSE)


#############
d <- read.csv("cache/chrp_TE.csv")
d$x <- as.numeric(as.character(gsub("V", "", d$var)))
d$feature <- as.character(d$feature)

d$class <- gsub(".*TE_|_.*", "", d$fid)
