...
### files
files <- list.files("cache/mcmc_revise/features", pattern="RData", full.names = T)
...
out$context <- gsub(".*interpop_", "", out$par)
out$ft <- gsub(".G_", "", out$context)
out$context <- gsub("_.*", "", out$context)
out$ft <- gsub("_.*", "", out$ft)

write.table(out, "cache/response_ne_ft.csv", sep=",", row.names=FALSE, quote=FALSE)
...
library("tidyr")
library(ggplot2)
df <- read.csv("cache/response_ne_ft.csv")
